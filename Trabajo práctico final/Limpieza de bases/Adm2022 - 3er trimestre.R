...
base <- read_excel("Trabajo práctico final/2022 ADMISIÓN_ANONIMIZADA.xlsx", sheet = "7 - 8 - 9")
...
# hay que cambiar la fecha de ts a formato dd/mm/YY

fecha_ts <- base$T.S.

fecha_ts[21] <- NA #26/7 Ausente

fecha_ts <- as.Date(as.numeric(fecha_ts), origin = "1899-12-30")

fecha_ts <- format(fecha_ts, "%d/%m/%Y")

fecha_ts[21] <- format("26/07/2022",format = "%d/%m/%Y")
...
adm22 <- data.frame(
  ...
  Entrevista_ts_asistencia = present_ts, # no hay registros
...
write_xlsx(adm22,"Trabajo práctico final/Limpieza de bases/2022 ADMISIÓN_LIMPIA_3.xlsx")
